...
function Shift_signup_allowed_angel(
    ...
    if (config('location_access_enabled') && $angeltype->requires_location_access && !$user->hasAccessTo($shift->location_id)) {
...
function Shift_signup_allowed(
    ...
    if ($shift->isCancelled() && !auth()->can('user_shifts_admin')) {
        $free_entries = Shift_free_entries($needed_angeltype, $shift_entries);
        return new ShiftSignupState(ShiftSignupStatus::CANCELLED, $free_entries);
    }
